...
function getCurrentColorTheme(
  targetColorTheme: ColorThemeOption = 'no-preference',
  ...
  return (() => {
    if (
      targetColorTheme &&
      COLOR_SCHEMES.includes(targetColorTheme) &&
      targetColorTheme !== 'no-preference'
    ) {
      return {
        name: targetColorTheme,
        query: matchMedia(`(prefers-color-scheme: ${targetColorTheme})`),
        theme: getTheme(targetColorTheme),
      }
    }
    ...
    return {
      name: DEFAULT_TARGET_COLOR_SCHEME,
      query: matchMedia(`(prefers-color-scheme: no-preference)`),
      theme: getTheme(),
    }
  })()
}
...
export default function useColorTheme({
  targetColorTheme,
  syncWithSystem = true,
}: UseColorThemeProps): ColorTheme | undefined {
